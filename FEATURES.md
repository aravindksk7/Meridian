...
> Last updated: 2026-06-23 (22 features implemented)
...
| 2.3 | [Minimap](#23-minimap) | M | No | `[x]` | Live preview navigator |
| 2.4 | [Relationship Table Panel](#24-relationship-table-panel) | M | No | `[-]` | |
| 2.5 | [Annotation / Sticky Note Layer](#25-annotation--sticky-note-layer) | M | No | `[x]` | Canvas labels and review pins |
| 2.6 | [Animated Edge Motion](#26-animated-edge-motion) | S | No | `[x]` | Appearance setting |
...
| 5.3 | [Version Snapshots](#53-version-snapshots) | M | No | `[x]` | Named snapshots, compare, restore |
...
| # | Feature | Complexity | New library? | Status | Notes |
|---|---------|-----------|-------------|--------|-------|
| 6.1 | [Command Palette (Ctrl+K)](#61-command-palette) | M | No | `[x]` | Templates, nodes, export, navigation |
...
| 6.4 | [Multi-node Selection & Bulk Ops](#64-multi-node-selection--bulk-ops) | L | No | `[x]` | Shift-select, arrange, group, bulk edit |
...
| 6.7 | [Nested Domain Subdomains](#67-nested-domain-subdomains) | M | No | `[x]` | |
| 6.8 | [Integration Layer Icons](#68-integration-layer-icons) | S | No | `[x]` | |

---

## 7. Figma-style Workspace

| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 7.1 | Live Preview Properties | `[x]` | Canvas, zoom, modes, devices, snap, minimap, quality |
| 7.2 | Component Properties | `[x]` | Boolean, variant, swap, text, number, colour, state |
| 7.3 | Layers Panel | `[x]` | Searchable groups, nodes, edges, annotations |
| 7.4 | Design Tokens | `[x]` | Theme-aware palette, type, spacing, radii, shadows, edges |
| 7.5 | Presentation Mode | `[x]` | Full-canvas view and saved view navigation |
| 7.6 | Comments and Review Pins | `[x]` | Node/canvas feedback with resolution state |
| 7.7 | Responsive Preview | `[x]` | Built-in/custom viewports and density modes |
| 7.8 | Version History | `[x]` | Sanitized compare and reversible restore |
...
A small overview window in the bottom-right of the preview showing the whole rendered diagram with a highlighted viewport rectangle.

**Implementation**
- After each successful render, clone `#mermaidOutput svg` into `#previewMinimap`
- Scale the clone into the minimap body while preserving the preview's scrollable dimensions
- Compute the viewport rectangle from `#previewScroll` scroll offsets and client size
- Click or drag inside the minimap to compute target scroll offsets and navigate the live preview
- Scroll, zoom, resize, and render events refresh the viewport rectangle
- Collapse / expand the minimap body with the header button

**Files changed**: `meridian.html` — minimap HTML/CSS, render callbacks, `refreshMinimap()`, `updateMinimapViewport()`, `navigateMinimap()`
...
### 2.6 Animated Edge Motion
**Status**: `[x]`

Optional moving-arrow overlays for rendered edges, controlled from Appearance > Diagram.

**Implementation**
- Add `edgeAnimation` to diagram settings with Off / Slow / Fast options
- After Mermaid or Design Board render, append a separate SVG overlay group with arrow glyphs animated along existing edge paths
- Preserve the generated Mermaid code and original edge path count
- Remove overlays when disabled and skip animation for users who prefer reduced motion

**Files changed**: `meridian.html` — Appearance settings UI, settings persistence, `applyEdgeAnimation()` render hook
...
Click a subgraph control or title in the preview to collapse the group to a colorful summary node, then expand it again from the live preview.

**Implementation**
- `_collapsedGroups: Set<groupId>` global state, persisted to localStorage `meridian_collapsed_v1`
- Shared palette helpers style `env:`, `net:`, `domain:`, and `subdomain:` groups consistently in expanded and collapsed states
- After each render, resolve Mermaid cluster IDs and inject SVG Collapse / Expand controls with keyboard support
- In generated Mermaid code, replace each collapsed group with a styled proxy node such as `▶ Production (5 nodes)`
- Redirect external relationships to the proxy while omitting internal child-to-child edges
- The collapsed proxy is generated from parser state and never written to Smart Input

**Files changed**: `meridian.html` — `applyGroupCollapse()`, group visual helpers, SVG group controls, localStorage persistence

---

### 6.7 Nested Domain Subdomains
**Status**: `[x]`

`subdomain:` blocks can be nested inside `domain:` blocks to show capability or team groupings within a larger business domain.

**Implementation**
- Added `subdomain`, `subdom`, `subteam`, and `sd` prefixes to the parser, highlighter, autocomplete, entity picker, inspector, and settings token-colour grid
- Tracks `parser.subdomains` and `parser.subdomainParents` so child groups render inside the parent domain cluster
- Subdomain children also count as parent-domain members for palette styling, inspector membership, and collapse routing
- Collapse / expand controls work independently for subdomains in the live preview
- Flowchart, Design Board, Sequence, and Class output paths understand `subdomain` as a container/entity type where applicable

**Files changed**: `meridian.html`, `tests/subdomain-groups.spec.js`

---

### 6.8 Integration Layer Icons
**Status**: `[x]`

The General icon catalog includes an Integration category for event streaming, enterprise integration, and managed file transfer tools.

**Implementation**
- Added Confluent Kafka, Kafka Connect, Schema Registry, Debezium CDC, Apache Camel, MuleSoft, Boomi, IBM MQ, TIBCO, Cleo Integration, Sterling B2B, CrushFTP, SFTPGo, FileZilla Server, and GoAnywhere MFT
- Regenerated the embedded `CLOUD_ICONS.general` catalog and Iconify override map from `icons-data/general.json`
- Covered catalog presence and picker insertion with Playwright tests

**Files changed**: `icons-data/general.json`, `meridian.html`, `tests/integration-icons.spec.js`
